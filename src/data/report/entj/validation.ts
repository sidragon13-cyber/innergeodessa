...
export interface EntjReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateEntjCompleteReport(
  report: CompletePersonalityReportDefinition =
    ENTJ_COMPLETE_REPORT,
): EntjReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ENTJ") {
    errors.push(
      `personalityType must be ENTJ; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ENTJ report must contain all 18 canonical sections.",
    );
  }
