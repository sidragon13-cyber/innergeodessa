...
export interface IstjReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateIstjCompleteReport(
  report: CompletePersonalityReportDefinition =
    ISTJ_COMPLETE_REPORT,
): IstjReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ISTJ") {
    errors.push(
      `personalityType must be ISTJ; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ISTJ report must contain all 18 canonical sections.",
    );
  }
