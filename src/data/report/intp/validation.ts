...
export interface IntpReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateIntpCompleteReport(
  report: CompletePersonalityReportDefinition =
    INTP_COMPLETE_REPORT,
): IntpReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "INTP") {
    errors.push(
      `personalityType must be INTP; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "INTP report must contain all 18 canonical sections.",
    );
  }
