...
> = {
  ...
  "motivation-and-needs": [
    block(
      ...
      "精通、自主与有意义的进展",
      "INTJ 往往更容易被复杂问题、独立判断的空间、提升能力的机会，以及值得长期投入的目标所激发。当目标本身具有意义，并且实现方式不受不必要的惯例限制时，投入感通常会明显增强。并不是每个 INTJ 都追求正式权力或独自工作。更深层的需要通常是能够掌控自己的注意力，并拥有足够一致的环境，使持续投入不会不断被任意打断。",
    ),
    block(
      ...
      "服务于系统目标的自主性",
      "独立空间能够保护复杂架构思考所需要的连续注意力。INTJ 可能会抗拒只关注表面活动、却忽视推理过程和实际结果的监督。自主性在与透明约束、定期复盘以及结果责任结合时最具价值。缺少这些支点，自我主导也可能逐渐变成隔绝反馈。真正重要的问题是：哪些决策需要被保护的独立判断，哪些决策则会因为更早引入协作而得到改善。",
    ),
    block(
      ...
      "为艰难改进保持耐心",
      "当目标足够重要时，INTJ 往往能够接受较长的学习曲线，并愿意投入那些短期内不容易获得认可的基础工作。这有助于形成深厚专业能力、严谨设计，以及超越短期项目周期的长期改进。但这种优势需要周期性地与用户、执行者或其他现实证据保持接触。坚持只有在方向仍然可靠时才有价值，而不是因为改变模型会让之前投入的能力显得被浪费。",
    ),
    block(
      ...
      "当能力成为保护机制",
      "对能力和掌控感的追求，有时会变成抵御依赖、不确定性或公开犯错的一种方式。INTJ 可能直到问题已经发展得较深才寻求帮助，也可能排斥自己暂时无法胜任的任务，或者不断提高标准，使成果始终处于无法真正暴露和接受评价的状态。能力依然重要，但当学习、关系和贡献允许包含尚未完成的知识以及人与人之间的相互依赖时，自我认同会更加稳定而有韧性。",
    ),
    block(
      ...
      "用外部结果重新定义进展",
      "面对一项重要投入，同时明确内部质量标准以及证明工作真正产生价值的外部证据。增加对学习效果、采用程度、可维护性和协作质量的衡量。判断哪些地方必须追求卓越，哪些地方更适合先推出可用版本以获得更真实的信息。在独自解决问题之前，主动向可信赖的同事分享一个尚未确定的问题。这样可以让能力追求始终与现实结果相连，而不是通过不断打磨来推迟接受外界检验。",
    ),
  ],
  "information-processing": [
    block(
      ...
      "模式、趋势与系统逻辑",
      "INTJ 往往会关注现象背后的结构、未来可能产生的影响，以及少数能够组织大量表面事件的关键变量。当信息能够帮助澄清发展趋势，或改善对整个系统的内部模型时，它通常最具价值。这种倾向有利于形成战略前瞻，但前提是抽象思考始终与当前数据、运营细节和其他可能解释保持联系。模式识别能够产生假设，却不能替代证据本身。",
    ),
    block(
      ...
      "将复杂信息压缩为模型",
      "你可能会把大量信息整合成一个相对简洁的解释，用来说明各部分之间如何关联，以及整个系统正在向什么方向发展。优秀的模型能够降低噪音并帮助确定优先级。但任何压缩都会丢失细节，而其中某些细节可能恰好揭示另一种运行机制。记录模型无法充分解释的事实，并清楚区分直接观察与推断，避免一个过于整洁的叙事掩盖真实的不确定性。",
    ),
    block(
      ...
      "看见二阶后果",
      "INTJ 常会自然地追问：今天的一个决定，会让未来哪些结果更容易发生？哪些依赖关系会增强？哪些激励会改变？哪些能力一旦失去就难以重新建立？这种思考能够帮助团队避免那些短期看起来很有吸引力、长期却代价高昂的解决方案。只有当未来影响被转化为可以观察的指标，并且概率被如实表达而不是包装成必然预见时，这种优势才真正具有实践价值。",
    ),
    block(
      ...
      "过滤掉不方便的现实",
      "一旦整体模式看起来足够清晰，不符合模型的数据就可能被视为例外、政治因素，或者过于琐碎的运营问题。但一个令人不舒服的细节，有时恰恰能够暴露错误假设，或揭示架构中忽略的相关方成本。真正的风险并不是抽象思考本身，而是让模型反过来决定哪些证据才算有效。对异常现象应有意识地保留，直到它们获得合理解释，而不是为了维持战略图景的整洁而将其排除。",
    ),
    block(
      ...
      "同时维护模型与异常记录",
      "写下当前的系统模型、三个最重要的假设，以及如果模型正确时应该出现的关键指标。同时保留一份与模型不一致的观察记录，并与真正接近执行现场的人定期复盘。当异常逐渐形成稳定模式时，及时修改架构并记录修改原因。这样既能保持长期思考的一致性，也能确保现实证据确实拥有改变方向的能力。",
    ),
  ],
  "decision-making": [
    block(
      ...
      "标准、后果与审慎承诺",
      "INTJ 往往偏好以明确目标、内部一致性、现实证据和长期后果为基础作出决定。在真正承诺之前，你通常希望有足够时间理解整个架构；一旦方向确定，又可能较为稳定地坚持下去。这有助于减少反应式的频繁切换。但良好的判断仍然需要纳入人的影响、可逆性以及未知因素。如果决策标准忽略了实际采用、伦理影响或他人掌握的重要信息，即使逻辑本身非常一致，最终选择仍可能失败。",
    ),
    block(
      ...
      "在内部完成决策收敛",
      "你可能习惯先在内部比较各种方案，直到一个优先方向逐渐形成之后才开始沟通。这样能够保护专注，但也会让协作者看不到决策形成的过程，他们最终接收到的可能只是结论，而不是支撑结论的证据和取舍。更早地让推理过程可见，并不等于追求无休止的共识。提前分享标准和不确定性，可以让其他人在修改成本还不高的时候帮助改善整个模型。",
    ),
    block(
      ...
      "抵抗短期压力",
      "当短期激励更偏向便利、速度或表面效果时，INTJ 仍可能坚持一个具有长期必要性的选择。通过把决定与整个系统的后果联系起来，你能够保护重要投资、质量标准或未来能力。这种优势需要明确的复盘触发条件，否则坚持可能逐渐变成僵化。一个决定之所以保持稳定，应该是因为证据仍然支持它，而不是因为改变方向会挑战自己作为战略制定者的身份认同。",
    ),
    block(
      "intj-decisions-risk",
      "risk",
      ...
      "优化了错误的标准",
      "当主导决策的标准本身不完整时，再精确的分析也可能产生糟糕结果。效率、可扩展性或逻辑优雅可能被不断优化，但信任、可访问性、时机或转型成本却被排除在模型之外。这些被忽略的影响最终往往会以执行失败的形式重新出现。在最终决定之前，应该追问：谁来定义成功？谁承担成本？哪些定性证据也应该真正影响选择？",
    ),
    block(
      ...
      "让决策架构变得可见",
      "记录目标、判断标准、关键假设、最强替代方案，以及什么样的证据会触发重新评估。在最终决定之前，把这些内容分享给掌握不同信息的人，并明确哪些部分已经确定、哪些部分仍可调整。这样既能保留独立判断，也能让推理过程接受检验，减少突发变化，并帮助协作者在理解原因的基础上执行，而不是仅仅服从结论。",
    ),
  ],
  "communication": [
    block(
      ...
      "简洁、结构化且有选择地表达",
      "INTJ 往往更偏好目标明确、推理一致，而且内容足以值得投入注意力的沟通方式。你可能会先在内部完成大量思考，再选择性地表达，而不是把每一步过程都说出来。这能够让沟通更加集中，但其他人无法自动推断出你头脑中的完整模型。有效沟通需要把内部架构转化为可理解的背景、假设、决定以及对证据的邀请，而不意味着必须持续进行高强度的社交表达。",
    ),
    block(
      ...
      "推理中被省略的中间过程",
      "由于 INTJ 往往已经在内部连接了大量推理步骤，对外说明时可能直接从问题跳到结论，却省略中间的逻辑桥梁。听者因此可能觉得结论突然，或者缺乏支持。解决办法不是简单增加表达量，而是让关键逻辑链变得可见：你观察到了什么？这些事实说明什么？还有哪些不确定性？为什么最终建议会由这些信息推导出来？",
    ),
    block(
      ...
      "沟通不足与默认他人已经理解",
      "你可能会一直等到模型足够完善之后才开始分享，随后又因为别人缺乏背景信息，或者重复提出你早已在内部解决的问题而感到挫败。沉默可能被他人理解为疏离、认同，甚至缺乏兴趣。在压力状态下，过于简洁的纠正也可能听起来像是否定。更有效的做法是在足够早的时候沟通，让其他人仍有机会影响工作，而不是只在架构已经完成之后接收结果。",
    ),
    block(
      ...
      "分享尚未完成的模型",
      "可以采用一个简洁结构进行沟通：目标、当前模型、关键证据、不确定性，以及你希望得到的具体输入。明确说明这仍然是一份草案，使反馈不会被理解为要求立即形成最终结论。发生分歧时，解释究竟是哪个假设或标准不同，而不是只说对方结论错误。这样能够让严谨性成为协作工具，并减少不必要的人际误读。",
    ),
    block(
      ...
      "别人真正看见了什么？",
      "回顾一个你最近觉得答案非常明显的决定。协作者实际上获得了哪些信息？哪些逻辑连接只存在于你自己的内部模型中？你是否足够早地邀请了能够改变方向的证据？即使表达方式并不精确，对方提出的哪些情绪性或现实性担忧其实仍然重要？一个清晰的架构只有在其他人能够看见、检查并真正使用时，才会产生持续影响。",
    ),
  ],
  "teamwork-and-leadership": [
    block(
      ...
      "构建能力与方向",
      "在团队中，INTJ 往往通过明确长期方向、重新设计系统、保护关键注意力以及提前识别依赖关系来创造价值。你的领导方式可能更安静，也更偏向通过架构和原则产生影响，而不是依靠强烈的社交存在感。你通常更愿意与能力可靠、拥有实际自主权的同事合作。高质量团队协作要求把标准和推理过程变得可见，主动吸收一线专业知识，并认识到协调并不是战略工作的干扰，而是战略真正落地的一部分。",
    ),
    block(
      ...
      "有选择地进行协作",
      "当他人能够提供真正的专业能力或有价值的挑战时，INTJ 往往可以进行非常深入的协作；但缺乏重点的群体过程可能迅速消耗你的精力。有选择的协作有助于保护质量和注意力，但也可能让你错过通过非正式交流获得的信息，或忽略那些表达不够简洁但掌握重要事实的人。可以围绕明确问题和清晰决策权来设计协作，同时保留足够空间，让不以你偏好方式出现的证据也能够进入判断过程。",
    ),
    block(
      ...
      "为独立判断设计系统",
      "成熟的 INTJ 能够建立一套框架，让其他人在不需要持续获得中央批准的情况下，也能作出相对一致的决定。清晰的原则、接口和反馈机制可以在保持整体一致性的同时，把能力分布到更多人手中。这种领导方式比个人控制更容易扩展。它真正有效的前提，是系统允许适度的本地调整，同时每个人都清楚：当某条规则已经不再服务于目标时，应该如何提出挑战和修改。",
    ),
    block(
      "intj-teamwork-risk",
      "risk",
      ...
      "从不完美的协作中退出",
      "当会议效率低下或推理质量较弱时，INTJ 可能选择退出并独自解决问题。这样有时确实能够快速得到更好的设计，但也可能降低团队所有权、共同学习以及接触一线运营知识的机会。反复退出还会进一步强化“协作没有多少价值”的判断。在彻底放弃协作之前，先尝试改善协作结构：缩小讨论问题、提前准备证据，并明确分配决策责任。",
    ),
    block(
      ...
      "建立可以被挑战的架构",
      "明确团队所遵循的战略原则、约束条件和关键接口，并在这些边界内给予负责人真正的决策权，同时建立基于证据的复盘节点。可以指定一名挑战者专门检验关键假设，并在最终确定标准之前纳入一线执行者的意见。目标应该是建立一个其他人能够理解、调整并持续改进的系统，而不是一个只有原始设计者在场时才能正常运作的体系。",
    ),
  ],
  "career-environment": [
    block(
      ...
      "深度、自主与长期影响",
      "INTJ 往往更偏好能够奖励独立判断、复杂系统思考、持续学习和长期改进的工作环境。战略、科技、研究、设计、政策或专业领导岗位可能提供这些条件，但任何职业都不属于某一种人格类型。真正的匹配取决于兴趣、专业能力、价值观、资源以及组织现实。更重要的问题是：这份工作是否允许你进行足够深入的思考，同时又能把个人分析转化为具有现实意义的外部结果。",
    ),
    block(
      ...
      "需要受到保护的专注时间",
      "频繁中断以及偏重表面表现的活动，会让复杂工作的系统模型难以真正形成。INTJ 往往需要成块、不被打断的时间，以及清晰的决策责任边界。但完全隔离并不是答案，因为战略仍然需要持续接触用户、同事和不断变化的现实证据。真正有支持性的环境，会在深度分析与有明确目的的协作之间形成节奏，而不是把随时在线或完全独立中的任何一种视为理想状态。",
    ),
    block(
      "intj-career-example",
      "example",
      ...
      "能够奖励架构思维的工作",
      "系统设计、研究战略、产品架构、组织重构、政策分析或长期规划等工作，往往能够发挥把原则与后果连接起来的能力。但即使职位名称完全相同，实际工作内容也可能差异很大。判断职业匹配时，应关注日常工作是否包含真正的问题所有权、获取证据的机会、改进结构的权限，以及从设计到执行全过程中的责任，而不是只依赖职业刻板印象。",
    ),
    block(
      "intj-career-risk",
      "risk",
      ...
      "等待一个完美系统",
      "较高标准可能让现实中不够完善的组织显得不值得投入，而想象中的未来岗位却总是比任何真实环境更加一致。INTJ 可能因此一直等到条件足够理想才愿意真正发挥影响，或者不断重新设计职业路径却很少进行现实测试。成长往往意味着在一个有限系统内部逐步改善它。需要区分真正违背核心价值的约束，与所有重要工作中都会存在的普通模糊、组织政治和日常维护。",
    ),
    block(
      ...
      "评估战略匹配度",
      "可以从问题深度、自主程度、证据质量、时间跨度、执行责任以及是否能够获得高质量挑战等维度评估一份工作。了解组织如何复盘决策，以及长期工作能否在短期压力下继续获得支持。同时也要识别产生实际影响所需要的沟通与关系能力。选择一个真正重视架构思维的环境，然后持续建立让其他人能够理解并采用这些架构的能力。",
    ),
  ],
  "relationship-dynamics": [
    block(
      ...
      "有选择的信任与审慎承诺",
      "INTJ 往往会把较深的投入集中在相对少数的关系中，并重视诚实、思想上的尊重、可靠性以及独立思考的空间。关心可能更多通过解决问题、提前规划或持续履行承诺来表达，而不是频繁进行情绪性的描述。人与人之间差异很大，人格类型代码无法解释依恋方式或亲密关系的全部内容。当内在的忠诚能够通过对方真正可以识别的方式表现出来时，关系通常会更加稳固。",
    ),
    block(
      ...
      "内在承诺与外在模糊",
      "INTJ 可能认为持续陪伴、实际帮助或认真思考已经能够清楚表达关心，但另一方可能更需要直接的欣赏、情绪回应或更频繁的联系。这两种表达方式本身并没有高低之分。与其只使用自己觉得最真诚的方式，不如主动了解对方究竟如何感受到关心。适当的表达转换不会削弱真实性，反而能让原本存在于内心的承诺更容易被对方看见。",
    ),
    block(
      ...
      "无需表演的深度关系",
      "成熟的 INTJ 往往能够提供稳定、坦诚的反思、对自主性的尊重，以及共同面对长期困难问题的意愿。你可能不喜欢表面的安慰，而会认真对待对方真正重视的目标。当分析是在被需要时提供，并且情绪没有被当作无意义噪音时，这种方式能够建立很强的信任。深层连接也意味着能够陪伴一个暂时无法被解决或优化的体验，而不是立即把它转化成问题。",
    ),
    block(
      ...
      "退回到自我独立之中",
      "当感到被误解、失望或负荷过重时，INTJ 可能退回到个人分析中，并减少沟通。这能够暂时恢复控制感，却也迫使另一方自行解释沉默的含义。你甚至可能在真正表达担忧之前，就已经在内部形成了关于这段关系的结论。当独立性消除了修复关系的可能时，它就已经转变成孤立。需要空间时，应说明为什么需要、空间的作用是什么，以及什么时候重新恢复联系。",
    ),
    block(
      ...
      "让内在立场变得可见",
      "在提出解决方案之前，先尝试表达一项欣赏、一项不确定性和一个真实需要。发生冲突时，说明你正在使用怎样的理解模型，同时询问对方如何理解当前情况。双方可以提前约定需要空间时如何处理，以及之后如何重新建立联系。这并不要求持续暴露所有内在感受，而是提供足够的信息，让信任可以在不依赖猜测的情况下运作。",
    ),
  ],
  "change-and-adaptation": [
    block(
      ...
      "调整并适应系统架构",
      "INTJ 往往会通过预测未来条件，并在真正发生变化之前建立一条相对清晰的路径来应对转型。当现有系统已经无法满足长期需求时，你可能成为推动变革的重要力量。但如果变化直接否定了一个已经投入大量思考建立起来的模型，或者变化本身缺乏可理解的目的，适应就会变得更加困难。下方动态内容会进一步说明，相邻偏好的可用性如何使规划方式变得更迭代、更务实或更具协作性。",
      [
        {
    ...
    block(
      ...
      "修正模型并不等于放弃愿景",
      "由于战略模型通常连接了大量决定，修改其中一个关键假设时，可能会让人感觉整个架构都在被动摇。INTJ 因此可能在证据已经发生变化之后，仍然维护原来的方向。需要把底层目标与当前设计区分开来。模型可以改变，而更深层的目标依然保持不变。很多时候，主动修正一个不再准确的模型，比坚持一个逻辑完整却与现实偏离的预测更能体现真正的战略能力。",
    ),
    block(
      ...
      "提前考虑变化的二阶影响",
      "INTJ 可能注意到，一次转型不仅改变当前实施计划，也会进一步改变激励机制、组织能力以及未来可选择的路径。这种思考有助于形成更负责任的实施顺序，并保护其他人容易忽略的重要资源。当真正受到变化影响的人也参与检验这些预测时，这项优势最有价值。变革架构不仅应该描述最终系统在概念上的优点，也必须纳入人们实际经历的转型成本。",
    ),
    block(
      "intj-change-risk",
      "risk",
      ...
      "隐藏在战略一致性背后的僵化",
      "即使证据已经发生变化，不愿修改方向也可能被解释成是在保护长期目标。INTJ 有时会选择性地把挫折理解为执行不到位，而不是重新检查原有假设是否有问题。这会延迟学习，也可能把越来越多的压力转移给其他人。最好在实施之前就明确失败标准和复盘条件，使未来的调整成为正常的系统机制，而不需要等到承认整个愿景错误之后才能发生。",
    ),
    block(
      ...
      "把战略当作版本化模型",
      "把战略计划视为一个持续迭代的版本化模型。记录关键假设、预期指标、复盘日期，以及哪些部分属于稳定原则、哪些仍然处于实验状态。每次复盘时，明确现实分别验证了什么、否定了什么，以及哪些问题仍未得到答案。邀请一个相对独立的人挑战你的解释。持续更新版本能够保留学习和方向，同时避免所谓的一致性逐渐变成对某一种具体实施方式的依赖。",
    ),
  ],
  "stress-and-recovery": [
    block(
      ...
      "收缩、控制与过度孤立",
      "在持续压力下，INTJ 可能变得更加退缩、缩小注意范围、加强内部规划，或者对低效率和外界干扰表现出异常强烈的批评。另一些时候，也可能通过冲动性的感官活动，或急于控制眼前细节来应对压力。不同人的压力反应差异很大，这些表现本身并不具有诊断意义。真正有价值的是识别自己最早出现的变化，例如灵活性下降、沟通减少、睡眠受影响，或看待问题的视角开始变窄。",
    ),
    block(
      ...
      "当独处不再带来恢复",
      "独处能够帮助恢复专注，但过度延长的孤立状态可能只是让同一个模型不断循环，而没有新的证据进入，也没有真正进行情绪处理。INTJ 可能在反复完善某种解释时感觉自己仍然很有生产力，却同时避免了那些可能改变模型的接触。恢复需要边界：先给自己足够的独处时间减少输入，然后重新回到具体行动、可信赖的交流，以及可以观察和验证的现实事实中。",
    ),
    block(
      ...
      "个人压力预警信号",
      "可能出现的信号包括取消社交联系、在没有新数据的情况下不断重做计划、对普通错误产生明显轻蔑、对模糊状态失去耐心，或者在试图仅靠思考解决整个系统时忽略身体作息。这些信号具有个体差异，并不是所有 INTJ 都会出现。观察在判断力、睡眠、健康或关系质量明显下降之前，自己通常最先发生哪些变化，也可以询问可信赖的人，他们最早会注意到你的哪些不同。",
    ),
    block(
      "intj-stress-risk",
      "risk",
      ...
      "用更多架构去解决压力",
      "当问题来自悲伤、疲惫、冲突或现实中无法完全控制的限制时，再增加一个计划可能只能暂时拉开距离，却不能真正处理正在经历的状态。INTJ 可能不断分析原因，同时与支持关系或身体需求保持脱节。战略思考以后仍然可以继续。眼前的恢复可能更需要先承认和表达感受、减少部分责任、接受暂时的不确定性，或者直接向他人寻求帮助。",
    ),
    block(
      ...
      "回到现实、主动沟通并缩小范围",
      "暂停一个并非必要的优化任务，先恢复睡眠和基本活动，并选择一件无需重新设计整个系统就能够完成的具体事情。告诉一个可信赖的人，你目前的承载能力有所下降，并明确说明真正需要什么支持。等状态恢复并获得新的证据之后，再重新审视之前形成的战略结论。如果痛苦状态持续存在，或已经明显影响正常功能，应寻求适当的专业支持，而不是简单用人格类型来解释这些问题。",
    ),
  ],
  "growth-roadmap": [
    block(
      ...
      "从个人愿景走向共享能力",
      "INTJ 的成长并不是放弃独立性或长期思考，而是让自己的模型可以被挑战，在完全确定之前就开始沟通，区分真正的卓越与完美主义，并允许关系证据和运营证据共同塑造系统架构。成长意味着战略能力范围扩大：你仍然可以保护深度思考，同时更早让他人参与，使他们能够帮助改善工作，并最终共同承担成果。下方的动态内容会进一步说明不同维度组合如何影响这一成长路径。",
      [
        {
    ...
    block(
      ...
      "把协作视为模型扩展",
      "协作并不意味着放弃独立判断，也不意味着必须接受缺乏重点的群体过程。高质量协作能够加入单个人无法获得的观察、约束条件和不同解释。成长任务在于设计一种协作方式：既有足够清晰的结构保护深度思考，也有足够开放的空间允许他人真正影响方向。如果所谓的咨询无论得到什么反馈都无法改变计划，那么它本质上只是沟通，而不是证据收集。",
    ),
    block(
      "intj-growth-risk",
      "risk",
      ...
      "伪装成“尚未准备好”的完美主义",
      "INTJ 可能不断推迟发布、授权或沟通，直到成果达到一个持续移动的个人标准。这能够维持控制感，却也阻止现实反馈真正改善最终结果。完美主义往往让注意力集中在那些在逻辑上容易看见的缺陷，而不是对结果影响最大的实际后果。需要明确真正必要的质量水平、继续延迟的成本，以及只有投入真实使用之后才能获得的证据。",
    ),
    block(
      ...
      "练习更早地暴露未完成想法",
      "选择一个模型、方案或担忧，在大约六成清晰时就分享给一个可信赖且真正相关的人。说明哪些部分已经稳定、哪些仍然不确定，以及什么样的反馈可能改变方向。不要立即解决每一个反对意见，可以先记录下来，再回到整体架构中统一复盘。每周重复一次，使“让思考过程可见”逐渐成为严谨思考的一部分，而不是对严谨性的威胁。",
    ),
    block(
      "intj-growth-action",
      "action",
      ...
      "每月进行一次假设复盘",
      "每个月选择一项重要战略，复盘其中的关键假设、异常现象、相关方影响以及执行证据。明确至少一个被现实进一步强化的判断，以及一个可信度有所下降的判断。把任何重要修改及时沟通出去，并明确指出促成修改的证据来源。这样能够把思维谦逊变成一种可见的实践，同时保留 INTJ 在长期一致性设计方面的优势。",
    ),
  ],
  "ninety-day-action-plan": [
    block(
      ...
      "90天共享架构实践",
      "利用接下来的90天，验证更早地让思考过程和未完成方案接受外部检验，是否能够改善一个真正重要的战略结果。选择一个既需要深度分析、又需要他人理解和采用的项目。继续保留专注工作的时间，但在预先设定的节点主动公开关键假设和阶段性决定。目标不是获得最大程度的共识，也不是形成毫无缺陷的计划，而是建立一个其他人能够理解、挑战并真正执行的更强架构。",
    ),
    block(
      ...
      "第1–30天 · 建模并公开",
      "写下目标、系统模型、关键假设、异常现象、相关方以及证据阈值。找到两位能够接触不同现实信息的人，在完整路线最终确定之前就向他们分享草案。分别请他们提供一个能够挑战当前判断的反例，以及一个实际执行中可能出现的问题。同时明确哪些质量标准必须从一开始就坚持，哪些可以通过早期版本在真实环境中进一步验证。",
    ),
    block(
      ...
      "第31–60天 · 构建并验证",
      "选择整个架构中杠杆效应最高的部分开始实施，并建立清晰的接口和衡量指标。保留连续的专注工作时间，然后与实际执行者或用户一起检查现实证据。记录哪些个人预测是准确的、哪些并不完整，以及哪些判断被事实证明错误。当模型发生变化时及时沟通，并在所有细节达到个人理想标准之前，至少发布一个真正可用的版本。",
    ),
    block(
      ...
      "第61–90天 · 分布能力并完成整合",
      "通过原则、约束条件和复盘触发机制转移真正有意义的决策权，而不是继续依赖逐步控制。完善必要文档，并邀请其他人尝试解释、使用或调整这套架构。评估实际结果、采用程度、可维护性，以及是否真正建立了他人的独立能力。根据实际发生的结果修改模型，而不是只检查执行过程是否忠实遵循了最初计划。",
    ),
    block(
      "intj-action-review",
      "reflection",
      ...
      "判断成长是否真实发生",
      "检查具体证据：是否更早分享了关键假设，是否保留了反例，是否发布了真正可用的版本，是否根据证据修改过决定，以及是否在不失去整体一致性的情况下把责任和能力分布给更多人。询问协作者，你的推理过程是否变得更容易理解和挑战。真正的成长来自现实条件下反复出现的新行为，而不是仅仅认同一个听起来有吸引力的战略型人格描述。",
    ),
    block(
      ...
      "保护思考深度，但不要重新回到孤立",
      "如果协作开始变得嘈杂和低效，优先改善协作结构，而不是直接放弃协作。缩小需要讨论的问题，提前要求准备证据，并区分咨询意见与最终决策权。如果完美主义开始拖延交付，就缩小范围，同时保留真正关键的质量标准。当独立思考与外部纠正能够共同成为同一个可靠流程的一部分时，这份90天计划才真正实现了目标。",
    ),
  ],
  methodology: [
    block(
      ...
      "如何使用本报告",
      "本报告基于类似 MBTI 的人格偏好结果，结合 EI、SN、TF、JP 四个维度、置信度数据、类型层级的静态内容以及匹配到的动态规则进行解释。报告用于描述可能存在的倾向，并帮助用户进行自我反思。它不是临床诊断、智力测试、能力评估，也不是对人格的科学绝对定义。任何单独章节都不应被作为重要就业、教育、关系、财务或健康决策的唯一依据。",
    ),
    block(
      ...
      "情境与个体差异",
      "即使同样获得 INTJ 结果，不同个体也可能因为文化、年龄、专业经验、责任、健康状况、价值观、环境和成长经历而表现出明显差异。当某个维度较为平衡或置信度较低时，相邻偏好同样容易被使用的可能性也会提高。长期、反复出现的真实生活证据应当比人格类型刻板印象获得更高权重。对于与你不相符的描述，应把它视为有价值的信息，而不是认为自己没有正确符合某个类型代码。",
    ),
    block(
      ...
      "动态解释机制",
      "生成报告时，系统会根据用户提供的维度得分和置信度模式选择相应的规则内容。已应用规则的元数据会记录具体插入了哪些动态调整，从而保证报告版本、内容版本和规则版本之间可以追溯。动态文本能够对宽泛的人格叙述进行进一步限定，但不能推断一个人的动机、个人历史、成熟程度、心理健康状况或未来结果。整个解释系统仍然是一种辅助反思的工具，而不是自动化的人格判断。",
    ),
    block(
      ...
      "版本记录与负责任的复盘",
      "报告版本：1.0.0。内容版本：1.0.0。规则版本：1.0.0。随着内容和规则持续演进，这些版本标识能够保留每一次结果生成时所使用的具体来源。当现实环境或自我理解发生变化时，可以重新查看报告，并将其中的解释与他人反馈和真实行为进行比较。最终只保留那些能够帮助形成更准确、更负责任自我理解的解释。",
    ),
  ],
};
