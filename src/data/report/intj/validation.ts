...
export interface IntjReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateIntjCompleteReport(
  report: CompletePersonalityReportDefinition =
    INTJ_COMPLETE_REPORT,
): IntjReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "INTJ") {
    errors.push(
      `personalityType must be INTJ; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "INTJ report must contain all 18 canonical sections.",
    );
  }
