import { ESFJ_COMPLETE_REPORT } from "../report";
import {
  DIMENSION_CODES,
} from "../../rules";
import type {
...
import { ESFJ_COMBINATION_RULES } from "./combination-rules";
import { ESFJ_CONFIDENCE_RULES } from "./confidence-rules";
import { ESFJ_DIMENSION_RULES } from "./dimension-rules";

export const ESFJ_REPORT_RULES:
...
export interface EsfjRuleSetValidationResult {
  valid: boolean;
  errors: string[];
}
...
const REQUIRED_AVERAGE_RULES = [
  {
    id: "esfj-confidence-average-low",
    minimum: 0,
    maximum: 15,
  },
  {
    id: "esfj-confidence-average-moderate",
    minimum: 16,
    maximum: 40,
  },
  {
    id: "esfj-confidence-average-strong",
    minimum: 41,
    maximum: 70,
  },
  {
    id: "esfj-confidence-average-very-strong",
    minimum: 71,
    maximum: 100,
  },
] as const;
...
const REQUIRED_BALANCED_RULES = [
  { id: "esfj-balanced-count-1", minimum: 1 },
  { id: "esfj-balanced-count-2", minimum: 2 },
  { id: "esfj-balanced-count-3", minimum: 3 },
] as const;
...
export function validateEsfjRuleSet(
  ...
  rules.forEach((rule) => {
    ...
    if (rule.personalityType !== "ESFJ") {
      errors.push(
        `Rule ${rule.id} personalityType must be ESFJ.`,
      );
    }
...
function collectTargetSlots(): Map<string, Set<string>> {
  return new Map(
    ESFJ_COMPLETE_REPORT.sections.map((section) => [
      section.id,
      new Set([
...
function validateExclusivePriority(
  ...
  const communityPriorities =
    ...
    new Set<number>();
  if (communityPriorities.has(rule.priority)) {
    errors.push(
      `Exclusive community ${rule.exclusiveGroup} must use unique priorities; duplicate ${rule.priority}.`,
    );
  }
  communityPriorities.add(rule.priority);
  priorities.set(rule.exclusiveGroup, communityPriorities);
}
...
function validateCombinationCoverage(
  ...
  if (combinationCount !== 8) {
    errors.push(
      `ESFJ rule set must contain exactly eight combination rules; found ${combinationCount}.`,
    );
  }
