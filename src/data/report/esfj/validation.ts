...
export interface EsfjReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateEsfjCompleteReport(
  report: CompletePersonalityReportDefinition =
    ESFJ_COMPLETE_REPORT,
): EsfjReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ESFJ") {
    errors.push(
      `personalityType must be ESFJ; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ESFJ report must contain all 18 canonical sections.",
    );
  }
