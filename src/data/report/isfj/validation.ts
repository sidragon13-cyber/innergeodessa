...
export interface IsfjReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateIsfjCompleteReport(
  report: CompletePersonalityReportDefinition =
    ISFJ_COMPLETE_REPORT,
): IsfjReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ISFJ") {
    errors.push(
      `personalityType must be ISFJ; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ISFJ report must contain all 18 canonical sections.",
    );
  }
