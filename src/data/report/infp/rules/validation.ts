...
import { INFP_COMPLETE_REPORT } from "../report";
import { INFP_COMBINATION_RULES } from "./combination-rules";
import { INFP_CONFIDENCE_RULES } from "./confidence-rules";
import { INFP_DIMENSION_RULES } from "./dimension-rules";

export const INFP_REPORT_RULES:
...
export interface InfpRuleSetValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateInfpRuleSet(
  ...
  if (rules.length !== 51) {
    errors.push(
      `INFP rule set must contain 51 rules; found ${rules.length}.`,
    );
  }
  ...
  rules.forEach((rule) => {
    ...
    if (rule.personalityType !== "INFP") {
      errors.push(
        `Rule ${rule.id} personalityType must be INFP.`,
      );
    }
    if (!rule.id.startsWith("infp-")) {
      errors.push(
        `Rule ${rule.id} must use the "infp-" prefix.`,
      );
    }
    ...
    rule.content.forEach((content) => {
      ...
      if (!content.blockId.startsWith("infp-")) {
        errors.push(
          `Rule ${rule.id} block id must use the "infp-" prefix.`,
        );
      }
...
function validateRequiredIds(
  ...
  requiredIds.forEach((id) => {
    if (!actualIds.has(id)) {
      errors.push(
        `INFP ${group} rules are missing "${id}".`,
      );
    }
  });
}
...
function collectTargetSlots(): Map<string, Set<string>> {
  return new Map(
    INFP_COMPLETE_REPORT.sections.map((section) => [
      section.id,
      new Set([
