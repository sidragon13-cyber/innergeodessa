...
export interface InfpReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateInfpCompleteReport(
  report: CompletePersonalityReportDefinition =
    INFP_COMPLETE_REPORT,
): InfpReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "INFP") {
    errors.push(
      `personalityType must be INFP; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "INFP report must contain all 18 canonical sections.",
    );
  }
...
function validateContentBlocks(
  ...
  report.sections.forEach((section, index) => {
    ...
    if (
      ...
      errors.push(
        `Premium section ${section.id} must include an reflection or summary block.`,
      );
    }
