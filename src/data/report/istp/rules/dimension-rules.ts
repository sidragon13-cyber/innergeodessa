...
export const ISTP_DIMENSION_RULES:
  readonly ReportRuleDefinition[] = CONFIGS.flatMap(
    (config) => {
      ...
      const balancedLocalized = localizeIstpRule(
        `istp-rule-${config.dimension.toLowerCase()}-balanced`,
      );
...
function createDirectionalRule(
  ...
  const localized = localizeIstpRule(
    `istp-rule-${dimensionId}-${preference.toLowerCase()}-${band}`,
  );
