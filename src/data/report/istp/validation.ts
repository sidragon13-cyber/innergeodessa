...
export interface IstpReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateIstpCompleteReport(
  report: CompletePersonalityReportDefinition =
    ISTP_COMPLETE_REPORT,
): IstpReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ISTP") {
    errors.push(
      `personalityType must be ISTP; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ISTP report must contain all 18 canonical sections.",
    );
  }
