...
export interface EnfpReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateEnfpCompleteReport(
  report: CompletePersonalityReportDefinition =
    ENFP_COMPLETE_REPORT,
): EnfpReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ENFP") {
    errors.push(
      `personalityType must be ENFP; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ENFP report must contain all 18 canonical sections.",
    );
  }
