...
export const ENFP_CONFIDENCE_RULES:
  readonly ReportRuleDefinition[] = [
    averageRule(
      ...
      "平均置信度低",
      "平均置信度处于 0–15，因此应把 ENFP 结果视为暂定假设。相邻偏好可能同样可用，角色、文化、压力、经验、语言或作答方式都可能显著影响外显模式。请用报告生成有关探索与承诺的问题，而不是捍卫标签；以不同情境中反复出现的行为核对每项解读，描述不符时优先相信生活证据。",
    ),
    averageRule(
      ...
      "平均置信度中等",
      "平均置信度处于 16–40。若干 ENFP 主题可能清晰可辨，但相邻偏好仍容易调用。可能性生成或口头探索会在某些情境中明显，在信任、专长、精力或责任不同的情境中较不显眼。阅读时寻找反复倾向而非一致行为，并辨别相反方法来自习得技能、情境适应，还是同样自然的选择。",
    ),
    averageRule(
      ...
      "平均置信度明显",
      "平均置信度处于 41–70，表示所测偏好可能在多个熟悉情境中稳定出现，因此 ENFP 主题可作为有用的组织框架。但一致性不等于更高的创造力、智力、成熟度或适应力。明显偏好会形成可重复优势与盲点；当探索变得分散、邀请削弱信任，或灵活性妨碍完成时，请运用反馈和相反偏好的练习。",
    ),
    averageRule(
      ...
      "平均置信度非常明显",
      "平均置信度处于 71–100，因此所测偏好可能在不同情境中都非常熟悉。这不表示更高的原创性、真实性、能力或未来成功。充满能量地创造可能性能邀请有意义的改变，过度使用却会分散承诺，或把他人的犹豫变成待重构的问题。在热情成为唯一有生命力的回应前，练习完成、安静倾听与具体落实。",
    ),
    balancedRule(
      ...
      "一个维度完全平衡",
      "至少一个维度完全平衡。ENFP 模式仍可能有用，但该字母不应被视为固定。行为会随角色、团队规模、信任、任务、精力或环境改变；这种灵活性能拓宽探索与执行选项，而非削弱结果。在对社交性、抽象思维、价值或自发性作出推断前，先观察什么条件会引出每一侧。",
    ),
    balancedRule(
      ...
      "两个维度完全平衡",
      "至少两个维度完全平衡，因此结果更可能描述混合且情境敏感的模式，而非僵化的四字母身份。你可能在互动与独立探索、想象与具体证据、不同决策标准，或开放与收束之间切换。请把 ENFP 报告当作比较框架，让重复行为、价值、技能和情境比类型刻板印象拥有更高权重。",
    ),
    balancedRule(
      ...
      "三个或更多维度完全平衡",
      "三个或更多维度完全平衡，使 ENFP 结果高度暂定且对情境敏感。多种方法可能同样可用，作答稍有变化便可能得到相邻代码。这会支持灵活性，却限制了对你如何探索、决定、沟通或完成工作的确定断言。请以生活证据检验每项报告主题，避免仅凭类型结果作出影响重大的选择。",
    ),
  ];
