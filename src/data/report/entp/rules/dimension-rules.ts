...
const CONFIGS: DimensionRuleConfig[] = [
  {
    ...
    firstDirectionZh:
      "偏 I 的 ENTP 可能先通过独立研究、书面模型和选择性交谈来探索，再公开提出新的框架。即使节省社交精力，思维范围仍可很广。这不会使 ENTP 结果失效，也不表示沟通较弱；它改变的是实验从何处开始，以及多少外部刺激才有帮助。",
    secondDirection:
    ...
    secondDirectionZh:
      "偏 E 的 ENTP 可能通过快速互动、提问、口头检验，以及接触不同的人或环境来发展可能性。交谈能产生能量并迅速揭示替代方案，代价可能是缺少独立整合，或被外部刺激带出过多方向。这项偏好描述能量模式，并不自动等于善于社交或富有影响力。",
    balancedDirection:
    ...
    balancedDirectionZh:
      "EI 平衡表示互动探索与独立概念工作都较容易调用。你可能在一种情境中公开生成选项，在另一种情境中退回独处以集中整合；角色、信任、团队规模和可用精力会决定哪种模式出现，而不表示整体模式矛盾。",
    guidance:
  ...
  },
  {
    ...
    firstDirectionZh:
      "偏 S 的 ENTP 可能从可观察事实、实际机制和直接实验中产生替代方案。重构会更关注改善事物实际如何运作，而非提出遥远的概念未来。这不会使 ENTP 结果失效，也不表示想象力较少；它让创造性质疑落在具体证据与可用细节上。",
    secondDirection:
    ...
    secondDirectionZh:
      "偏 N 的 ENTP 往往很快注意到正在浮现的模式、概念连接和当前框架之外的可能。这支持构想和战略重构，但可能低估运营限制或相互矛盾的细节。偏好只表示注意力通常从哪里开始，并不能证明想法原创、准确或有价值。",
    balancedDirection:
    ...
    balancedDirectionZh:
      "SN 平衡表示概念可能与实际证据可能同样容易调用。你既可能从宽广重构走向具体原型，也可能让亲手获得的发现生成新模型；专长、时间跨度和后果会决定哪类信息优先。",
    guidance:
  ...
  },
  {
    ...
    firstDirectionZh:
      "偏 F 的 ENTP 可能同时依据价值、正当性、利益相关者体验和概念连贯性来检验可能。重构会关注创造人们能够信任并采用的选项。这不会使 ENTP 结果失效，不会削弱分析能力，也不保证同理心；它改变的是哪些后果较早进入评估。",
    secondDirection:
    ...
    secondDirectionZh:
      "偏 T 的 ENTP 可能强调内部一致性、非个人标准、矛盾，以及竞争模型的解释力。这支持严谨质疑，但情绪信息或关系后果可能被放在次要位置。逻辑只是一种惯用标准，并非智力、客观性或不受偏见影响的证据。",
    balancedDirection:
    ...
    balancedDirectionZh:
      "TF 平衡表示逻辑连贯与人的后果都可能塑造判断，而没有一方持续主导。你可能精确质疑论证，同时关注过程是否维护尊严、信任与采用；不同情境会让不同标准走到前台。",
    guidance:
    ...
    guidanceZh:
      "主张重构前，同时评估它能否经受逻辑质疑，以及实施会如何影响人、信任、激励与责任。",
  },
  {
    dimension: "JP",
    firstPreference: "J",
    ...
    firstDirectionZh:
      "偏 J 的 ENTP 可能比常见的灵活刻板印象更偏好清楚的优先级、决策节点和完成结构。可能性生成可被导入有组织的实验与及时选择。这不会使 ENTP 结果失效；它可能提高一致性，也会带来偏爱模型过早结束探究的风险。",
    secondDirection:
    ...
    secondDirectionZh:
      "偏 P 的 ENTP 可能保留选择空间，随证据变化修订路线，并在探索仍有产出时抗拒收束。这支持适应性学习，但过多未闭环事项会分散责任，阻碍成果累积。该偏好描述对开放的取向，并不证明自发、富有创造力或可靠性较弱。",
    balancedDirection:
    ...
    balancedDirectionZh:
      "JP 平衡表示选择性结构与有意义的灵活并存。你可能设定明确成果和复查日期，同时保持方法可调，也可能先广泛探索再果断承诺。可逆性、后果和共同义务会决定何时收束有用。",
    guidance:
    ...
    guidanceZh:
      "把灵活方法与稳定承诺分开：开启更多可能性前，先定义成果、实验窗口、证据门槛和复查节点。",
  },
];
...
function createDirectionalRule(
  ...
  const ruleId = `entp-${dimensionId}-${preference.toLowerCase()}-${band}`;
