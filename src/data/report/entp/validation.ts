...
export interface EntpReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateEntpCompleteReport(
  report: CompletePersonalityReportDefinition =
    ENTP_COMPLETE_REPORT,
): EntpReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ENTP") {
    errors.push(
      `personalityType must be ENTP; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ENTP report must contain all 18 canonical sections.",
    );
  }
