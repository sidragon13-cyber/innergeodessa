...
export interface InfjReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateInfjCompleteReport(
  report: CompletePersonalityReportDefinition =
    INFJ_COMPLETE_REPORT,
): InfjReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "INFJ") {
    errors.push(
      `personalityType must be INFJ; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "INFJ report must contain all 18 canonical sections.",
    );
  }
