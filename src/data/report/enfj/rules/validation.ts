...
import { ENFJ_COMBINATION_RULES } from "./combination-rules";
import { ENFJ_CONFIDENCE_RULES } from "./confidence-rules";
import { ENFJ_DIMENSION_RULES } from "./dimension-rules";

export const ENFJ_REPORT_RULES:
...
export interface EnfjRuleSetValidationResult {
  valid: boolean;
  errors: string[];
}
...
const REQUIRED_AVERAGE_RULES = [
  {
    id: "enfj-confidence-average-low",
    minimum: 0,
    maximum: 15,
  },
  {
    id: "enfj-confidence-average-moderate",
    minimum: 16,
    maximum: 40,
  },
  {
    id: "enfj-confidence-average-strong",
    minimum: 41,
    maximum: 70,
  },
  {
    id: "enfj-confidence-average-very-strong",
    minimum: 71,
    maximum: 100,
  },
] as const;
...
const REQUIRED_BALANCED_RULES = [
  { id: "enfj-balanced-count-1", minimum: 1 },
  { id: "enfj-balanced-count-2", minimum: 2 },
  { id: "enfj-balanced-count-3", minimum: 3 },
] as const;
...
export function validateEnfjRuleSet(
  ...
  rules.forEach((rule) => {
    ...
    if (rule.personalityType !== "ENFJ") {
      errors.push(
        `Rule ${rule.id} personalityType must be ENFJ.`,
      );
    }
...
function collectTargetSlots(): Map<string, Set<string>> {
  return new Map(
    ENFJ_COMPLETE_REPORT.sections.map((section) => [
      section.id,
      new Set([
...
function validateCombinationCoverage(
  ...
  if (combinationCount !== 8) {
    errors.push(
      `ENFJ rule set must contain exactly eight combination rules; found ${combinationCount}.`,
    );
  }
