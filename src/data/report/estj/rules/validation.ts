import { ESTJ_COMPLETE_REPORT } from "../report";
import {
  DIMENSION_CODES,
} from "../../rules";
import type {
...
import { ESTJ_COMBINATION_RULES } from "./combination-rules";
import { ESTJ_CONFIDENCE_RULES } from "./confidence-rules";
import { ESTJ_DIMENSION_RULES } from "./dimension-rules";

export const ESTJ_REPORT_RULES:
...
export interface EstjRuleSetValidationResult {
  valid: boolean;
  errors: string[];
}
...
const REQUIRED_AVERAGE_RULES = [
  {
    id: "estj-confidence-average-low",
    minimum: 0,
    maximum: 15,
  },
  {
    id: "estj-confidence-average-moderate",
    minimum: 16,
    maximum: 40,
  },
  {
    id: "estj-confidence-average-strong",
    minimum: 41,
    maximum: 70,
  },
  {
    id: "estj-confidence-average-very-strong",
    minimum: 71,
    maximum: 100,
  },
] as const;
...
const REQUIRED_BALANCED_RULES = [
  { id: "estj-balanced-count-1", minimum: 1 },
  { id: "estj-balanced-count-2", minimum: 2 },
  { id: "estj-balanced-count-3", minimum: 3 },
] as const;
...
export function validateEstjRuleSet(
  ...
  rules.forEach((rule) => {
    ...
    if (rule.personalityType !== "ESTJ") {
      errors.push(
        `Rule ${rule.id} personalityType must be ESTJ.`,
      );
    }
...
function collectTargetSlots(): Map<string, Set<string>> {
  return new Map(
    ESTJ_COMPLETE_REPORT.sections.map((section) => [
      section.id,
      new Set([
...
function validateCombinationCoverage(
  ...
  if (combinationCount !== 8) {
    errors.push(
      `ESTJ rule set must contain exactly eight combination rules; found ${combinationCount}.`,
    );
  }
