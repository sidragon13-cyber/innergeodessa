...
export interface EstjReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateEstjCompleteReport(
  report: CompletePersonalityReportDefinition =
    ESTJ_COMPLETE_REPORT,
): EstjReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ESTJ") {
    errors.push(
      `personalityType must be ESTJ; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ESTJ report must contain all 18 canonical sections.",
    );
  }
