...
export interface EstpReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateEstpCompleteReport(
  report: CompletePersonalityReportDefinition =
    ESTP_COMPLETE_REPORT,
): EstpReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ESTP") {
    errors.push(
      `personalityType must be ESTP; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ESTP report must contain all 18 canonical sections.",
    );
  }
