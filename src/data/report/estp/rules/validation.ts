import { ESTP_COMPLETE_REPORT } from "../report";
import {
  DIMENSION_CODES,
} from "../../rules";
import type {
...
import { ESTP_COMBINATION_RULES } from "./combination-rules";
import { ESTP_CONFIDENCE_RULES } from "./confidence-rules";
import { ESTP_DIMENSION_RULES } from "./dimension-rules";

export const ESTP_REPORT_RULES:
...
export interface EstpRuleSetValidationResult {
  valid: boolean;
  errors: string[];
}
...
const REQUIRED_AVERAGE_RULES = [
  {
    id: "estp-confidence-average-low",
    minimum: 0,
    maximum: 15,
  },
  {
    id: "estp-confidence-average-moderate",
    minimum: 16,
    maximum: 40,
  },
  {
    id: "estp-confidence-average-strong",
    minimum: 41,
    maximum: 70,
  },
  {
    id: "estp-confidence-average-very-strong",
    minimum: 71,
    maximum: 100,
  },
] as const;
...
const REQUIRED_BALANCED_RULES = [
  { id: "estp-balanced-count-1", minimum: 1 },
  { id: "estp-balanced-count-2", minimum: 2 },
  { id: "estp-balanced-count-3", minimum: 3 },
] as const;
...
export function validateEstpRuleSet(
  ...
  rules.forEach((rule) => {
    ...
    if (rule.personalityType !== "ESTP") {
      errors.push(
        `Rule ${rule.id} personalityType must be ESTP.`,
      );
    }
...
function collectTargetSlots(): Map<string, Set<string>> {
  return new Map(
    ESTP_COMPLETE_REPORT.sections.map((section) => [
      section.id,
      new Set([
...
function validateCombinationCoverage(
  ...
  if (combinationCount !== 8) {
    errors.push(
      `ESTP rule set must contain exactly eight combination rules; found ${combinationCount}.`,
    );
  }
