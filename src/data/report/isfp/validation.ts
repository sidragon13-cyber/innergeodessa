...
export interface IsfpReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateIsfpCompleteReport(
  report: CompletePersonalityReportDefinition =
    ISFP_COMPLETE_REPORT,
): IsfpReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ISFP") {
    errors.push(
      `personalityType must be ISFP; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ISFP report must contain all 18 canonical sections.",
    );
  }
