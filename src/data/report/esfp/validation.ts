...
export interface EsfpReportValidationResult {
  valid: boolean;
  errors: string[];
}
...
export function validateEsfpCompleteReport(
  report: CompletePersonalityReportDefinition =
    ESFP_COMPLETE_REPORT,
): EsfpReportValidationResult {
  const errors: string[] = [];

  if (report.personalityType !== "ESFP") {
    errors.push(
      `personalityType must be ESFP; found "${report.personalityType}".`,
    );
  }
  ...
  if (
    ...
    errors.push(
      "ESFP report must contain all 18 canonical sections.",
    );
  }
