...
function formatPosition(position: ZodiacPosition): string {
  return [
    `${position.degree}°`,
    `${position.minute}′`,
    `${position.second}″`,
  ...
  return (
    <article className="rounded-[1.75rem] border border-[#ddd8cd] bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between gap-5">
        ...
        <div className="flex min-h-14 min-w-14 items-center justify-center rounded-full bg-[var(--color-surface-raised)] px-3 text-lg font-semibold text-[var(--color-primary)] border border-[var(--color-gold)]">
          {item.symbol}
        </div>
      </div>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {item.keywords.map((keyword) => (
          <span
            key={keyword}
            className="rounded-full border border-[#d8d2c6] bg-[#fbfaf7] px-3 py-1 text-xs font-medium text-[var(--color-text-secondary)]"
          >
            {keyword}
          </span>
        ))}
...
export default function ZodiacResultPage() {
  ...
  const dictionary = getZodiacResultDictionary(locale);
  ...
  const [result, setResult] = useState<AstrologyResultContract | null>(null);
  ...
  const displayStatus: ResultStatus = chartId ? status : "error";

  const displayResultError: ResultError | null = chartId
    ? resultError
    : "missingChartId";

  const corePositions = useMemo<DisplayPosition[]>(() => {
    if (!result) {
      return [];
    }
    ...
    return [
      {
        ...
        position: result.planets.sun.zodiac,
        retrograde: result.planets.sun.retrograde,
      },
      {
        id: "moon",
        ...dictionary.positions.moon,
        position: result.planets.moon.zodiac,
        retrograde: result.planets.moon.retrograde,
      },
      {
        id: "ascendant",
        ...dictionary.positions.ascendant,
        position: result.angles.ascendant.zodiac,
      },
    ];
  }, [dictionary.positions, result]);

  const personalPlanets = useMemo<DisplayPosition[]>(() => {
    if (!result) {
      return [];
    }
    ...
    return [
      {
        ...
        position: result.planets.mercury.zodiac,
        retrograde: result.planets.mercury.retrograde,
      },
      {
        id: "venus",
        ...dictionary.positions.venus,
        position: result.planets.venus.zodiac,
        retrograde: result.planets.venus.retrograde,
      },
      {
        id: "mars",
        ...dictionary.positions.mars,
        position: result.planets.mars.zodiac,
        retrograde: result.planets.mars.retrograde,
      },
    ];
  }, [dictionary.positions, result]);

  const chartAngles = useMemo<DisplayPosition[]>(() => {
    if (!result) {
      return [];
    }
    ...
    return [
      ...
      },
      {
        ...
        position: result.angles.descendant.zodiac,
      },
      {
        id: "midheaven",
        ...dictionary.positions.midheaven,
        position: result.angles.midheaven.zodiac,
      },
      {
        id: "imum-coeli",
        ...dictionary.positions.imumCoeli,
        position: result.angles.imumCoeli.zodiac,
      },
    ];
  }, [dictionary.positions, result]);
  ...
  return (
    <ResultShell>
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <ButtonLink href="/zodiac/test" variant="secondary">
      ...
      <ResultHeader
        ...
        className="mt-12 border-0 bg-[var(--color-primary-active)] px-7 py-10 text-[var(--color-on-primary)] sm:px-10 sm:py-14"
        title={
          <h1 className="max-w-4xl text-4xl font-semibold leading-tight sm:text-6xl">
            {dictionary.signNames[result.planets.sun.zodiac.sign]}
            {dictionary.header.title.sun}
            {dictionary.header.title.separator}
            {dictionary.signNames[result.planets.moon.zodiac.sign]}
            {dictionary.header.title.moon}
            {dictionary.header.title.separator}
            {dictionary.signNames[result.angles.ascendant.zodiac.sign]}
            {dictionary.header.title.rising}
          </h1>
        }
        description={
          <p className="text-[#d8dfdb]">{dictionary.header.description}</p>
        }
        badges={
      ...
      <section className="mt-14">
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-3">
          {corePositions.map((item) => (
            <PositionCard key={item.id} item={item} dictionary={dictionary} />
          ))}
        </div>
      </section>
      ...
      <section className="mt-16 rounded-[2rem] border border-[#ddd8cd] bg-white p-7 shadow-sm sm:p-9">
        ...
        <div className="mt-7 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            ...
            <p className="mt-2 font-semibold">{result.input.localDateTime}</p>
          </div>
          ...
          <div>
            ...
            <p className="mt-2 font-semibold">{result.input.timeZone}</p>
          </div>
          ...
          <div>
            ...
            <p className="mt-2 font-semibold capitalize">
              {dictionary.calculation.precision[result.input.timePrecision]}
            </p>
          </div>
    ...
    {premiumAccess === "unlocked" ? (
      ...
      >
        {locale === "zh"
          ? "登录并验证邮箱以查看完整报告"
          : "Sign in and verify email to view full report"}
      </ButtonLink>
    )}
      </section>
      ...
      <footer className="mt-10 border-t border-[#d8d2c6] pt-8 text-sm leading-6 text-[var(--color-text-secondary)]">
