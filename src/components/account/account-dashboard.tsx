...
export function AccountDashboard() {
  ...
  const loadResources = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const payload = (await response.json()) as DashboardResources;
    ...
    <div className="grid gap-10">
      <section className="border border-[#c8c2b5] bg-[#f7f4ec] p-7 md:p-9">
        <dl className="grid gap-6 md:grid-cols-2">
          ...
          <AccountField label={dictionary.dashboard.email} value={user.email} />
        ...
        <Button
          ...
          variant="secondary"
          loading={loggingOut}
          loadingLabel={dictionary.dashboard.loggingOut}
      ...
      <section>
        ...
        {historyStatus === "loading" || historyStatus === "idle" ? (
          <Notice className="mt-6">
            {dictionary.dashboard.history.loading}
          </Notice>
        ) : null}

        {historyStatus === "error" ? (
          <Notice className="mt-6">
            <p role="alert">{dictionary.dashboard.history.error}</p>
            ...
            >
              {dictionary.dashboard.history.retry}
            </Button>
          </Notice>
        ) : null}

        {historyStatus === "ready" && totalRecords === 0 ? (
          <Notice className="mt-6">{dictionary.dashboard.history.empty}</Notice>
        ) : null}

        {historyStatus === "ready" && totalRecords > 0 ? (
          <div className="mt-7 grid gap-8">
            <HistoryGroup
              title={dictionary.dashboard.history.personality}
              emptyLabel={dictionary.dashboard.history.empty}
            >
              {resources.personality.map((item) => (
                <HistoryCard
                  key={item.resourceId}
                  title={item.type}
            ...
            <HistoryGroup
              title={dictionary.dashboard.history.career}
              emptyLabel={dictionary.dashboard.history.empty}
            >
              {resources.career.map((item) => (
                <HistoryCard
                  key={item.resourceId}
                  title={item.code}
                  savedLabel={dictionary.dashboard.history.saved}
            ...
            <HistoryGroup
              title={dictionary.dashboard.history.zodiac}
              emptyLabel={dictionary.dashboard.history.empty}
            >
              {resources.zodiac.map((item) => (
                <HistoryCard
                  key={item.resourceId}
                  title={dictionary.dashboard.history.zodiac}
                  savedLabel={dictionary.dashboard.history.saved}
      ...
      <section>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {dictionary.dashboard.modules
            .filter((module) => module.id !== "test-history")
            .map((module) => (
              <article
                key={module.id}
  ...
  return (
    <section>
      ...
      {visibleRecords.length > 0 ? (
        ...
        <p className="mt-3 text-sm text-[#6d746b]">{emptyLabel}</p>
      )}
    </section>
  );
}
...
function HistoryCard({
  ...
  return (
    <article className="border border-[#c8c2b5] bg-[#f7f4ec] p-6">
      ...
      <p className="mt-3 text-sm text-[#596158]">{dateLabel}</p>

      {detail ? <p className="mt-2 text-xs text-[#6d746b]">{detail}</p> : null}
      ...
      {unavailable ? (
        <p className="mt-5 text-sm leading-6 text-[#7f3e33]">{unavailable}</p>
      ) : null}
    </article>
  );
}
